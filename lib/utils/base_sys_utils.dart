...
//    return new RegExp('(^[0-9])').hasMatch(str);
  ...
  * */
  ...
  static List<int> hex2ListInt(String? hex) {
    if (hex is! String) {
  ...
  static String listInt82Hex(List<int>? byteArr) {
  ...
  static Uint8List hex2Uint8List(String? hex) {
    if (hex is! String) {
      throw 'Expected string containing hex digits';
    }
    if (hex.length % 2 != 0) {
  ...
  static String uint82Hex(Uint8List? byteArr) {
    if (byteArr == null || byteArr.isEmpty) {
      return "";
    }
    Uint8List result = Uint8List(byteArr.length << 1);
