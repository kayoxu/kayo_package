...
class FlutterKeyboardController extends ValueNotifier<TextEditingValue>{
  final FlutterInputClient client;

  FlutterKeyboardController({TextEditingValue value,this.client})
  ...
  /// 下一个
  nextAction(){
    FlutterKeyboard.sendPerformAction(TextInputAction.next);
  }

  /// 换行
  newLineAction(){
    FlutterKeyboard.sendPerformAction(TextInputAction.newline);
  }

  ///发送其他Action
  sendPerformAction(TextInputAction action){
    FlutterKeyboard.sendPerformAction(action);
  }
}