...
class CityPickerState extends State<CityPickerWidget>
  ...
  List<TabTitle> _myTabs = [
    TabTitle(index: 0, title: "Please Select", city: null),
  ];
  ...
  void didChangeDependencies() {
    ...
    if (_myTabs.length == 1 && _cities.isEmpty) {
      _myTabs[0].title =
          KayoPackageLocalizations.of(context)?.pleaseSelect ?? "Please Select";
      setState(() {});
    }
  }
    ...
    for (int i = 0; i < tabIndex + 2; i++) {
      if (i == tabIndex + 1) {
        _myTabs.add(TabTitle(
            index: i,
            title: KayoPackageLocalizations.of(context)?.pleaseSelect ??
                "Please Select",
            city: city));
      } else {
