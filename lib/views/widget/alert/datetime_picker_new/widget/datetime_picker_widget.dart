...
class DateTimePickerWidget extends StatefulWidget {
  DateTimePickerWidget({
    ...
    this.onChange,
    this.onConfirm,
    this.onMonthChangeStartWithFirstDate = true,
  }) : super(key: key) {
...
class _DateTimePickerWidgetState extends State<DateTimePickerWidget> {
  ...
  late List<int> _yearRange,
      ...
      _minuteRange,
      _secondRange;
  late MPPageController _yearScrollCtrl,
      _monthScrollCtrl,
      _dayScrollCtrl,
      _hourScrollCtrl,
      _minuteScrollCtrl,
  ...
  late Map<String, MPPageController> _scrollCtrlMap;
  ...
  _DateTimePickerWidgetState(DateTime? minTime, DateTime? maxTime,
    ...
    _yearScrollCtrl =
        MPPageController(initialPage: _currYear - _yearRange.first);
    _monthScrollCtrl =
        MPPageController(initialPage: _currMonth - _monthRange.first);
    _dayScrollCtrl = MPPageController(initialPage: _currDay - _dayRange.first);
    _hourScrollCtrl =
        MPPageController(initialPage: _currHour - _hourRange.first);
    _minuteScrollCtrl = MPPageController(
        initialPage: (_currMinute - _minuteRange.first) ~/ _minuteDivider);
    _secondScrollCtrl =
        MPPageController(initialPage: _currSecond - _secondRange.first);
  ...
  MPPageController _findScrollCtrl(String format) {
    MPPageController? scrollCtrl;
  ...
  Widget _renderDatePickerColumnComponent({
    required MPPageController scrollCtrl,
    ...
    Widget columnWidget = Container(
      ...
      child: pk.CupertinoPicker.builder(
        backgroundColor: widget.pickerTheme.backgroundColor,
        scrollController: scrollCtrl,
        selectionOverlay: Container(
          decoration: BoxDecoration(color: Color(0x16666666)),
        ),
...
//    int value = _minuteRange.first + index;
  ...
  void _changeTimeRange() {
    ...
    if (monthRangeChanged) {
      ...
      _monthScrollCtrl.jumpToPage(monthRange.last - monthRange.first);
      if (currMonth < monthRange.last) {
        _monthScrollCtrl.jumpToPage(currMonth - monthRange.first);
      }
    }
    ...
    if (dayRangeChanged) {
      ...
      _dayScrollCtrl.jumpToPage(dayRange.last - dayRange.first);
      if (currDay < dayRange.last) {
        _dayScrollCtrl.jumpToPage(currDay - dayRange.first);
      }
    }
    ...
    if (hourRangeChanged) {
      ...
      _hourScrollCtrl.jumpToPage(hourRange.last - hourRange.first);
      if (currHour < hourRange.last) {
        _hourScrollCtrl.jumpToPage(currHour - hourRange.first);
      }
    }
    ...
    if (minuteRangeChanged) {
      ...
      _minuteScrollCtrl
          .jumpToPage((minuteRange.last - minuteRange.first) ~/ _minuteDivider);
      if (currMinute < minuteRange.last) {
        _minuteScrollCtrl.jumpToPage(currMinute - minuteRange.first);
      }
    }
