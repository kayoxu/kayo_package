...
class _DatePickerWidgetState extends State<DatePickerWidget> {
  ...
  Widget _renderDatePickerColumnComponent({
    ...
    required KayoPackageLocalizations localizations,
  }) {
    return Expanded(
      flex: 1,
      child: Container(
        ...
        child: CupertinoPicker.builder(
          ...
          itemBuilder: (context, index) => _renderDatePickerItemComponent(
              valueRange.first + index, format, localizations),
        ),
      ),
    );
  }
  ...
  Widget _renderDatePickerItemComponent(
    ...
    return Container(
      ...
      child: Text(
        DateTimeFormatter.formatDateTime(value, format, localizations),
  ...
  List<int> _calcMonthRange() {
    ...
    if (minYear == _currYear) {
      // current selected year equal min year
      minMonth = _minDateTime.month;
    }
    if (maxYear == _currYear) {
      // current selected year equal max year
  ...
  List<int> _calcDayRange() {
    ...
    if (minYear == _currYear && minMonth == _currMonth) {
      // current selected year and month equal min year and month
      minDay = _minDateTime.day;
    }
    if (maxYear == _currYear && maxMonth == _currMonth) {
      // current selected year and month equal max year and month
