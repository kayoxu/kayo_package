...
class DataPicker {
  ...
  static Future show<T>(BuildContext context,
    ...
    return _showDataPicker(context,
  ...
  static Future _showDataPicker<T>(BuildContext context,
      {bool showTitleActions = true,
      required List<T> datas,
      int selectedIndex = 0,
      List<int>? selectedIndexes,
    ...
    if (true == bottomSheet) {
      return showModalBottomSheet(
          ...
          backgroundColor: BaseColorUtils.darkWhite(context: context),
...
class _DataPickerState extends State<_DataPickerComponent> {
  ...
  Widget _renderDataPickerComponent(String suffixAppend) {
    return new Expanded(
      ...
      child: Container(
          ...
          decoration:
              BoxDecoration(color: BaseColorUtils.darkWhite(context: context)),
  ...
  String _localeDone() {
    return KayoPackageLocalizations.of(context)?.done ??
        KayoPackageLocalizations.of(context)?.confirm ??
        '确定';
  }

  String _localeCancel() {
    return KayoPackageLocalizations.of(context)?.cancel ?? '取消';
  }
}
...
class _BottomSheetSingleState<T> extends State<BottomSheetSingleWidget<T>> {
      ...
      children: [
        Container(
          ...
          child: Row(
            children: [
              TextView(
                KayoPackageLocalizations.of(context)?.cancel ?? '取消',
              ...
              Expanded(
                  child: TextView(
                widget.title ??
                    ((KayoPackageLocalizations.of(context)?.pleaseSelect ??
                            '请选择') +
                        (multipleChoice == true
                            ? '(${KayoPackageLocalizations.of(context)?.multipleChoices ?? '可多选'})'
                            : '')),
                maxLine: 2,
                textAlign: TextAlign.center,
                alignment: Alignment.center,
                color:
                    context.isDark ? Colors.white : BaseColorUtils.colorBlack,
                size: 17,
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              TextView(
                KayoPackageLocalizations.of(context)?.confirm ?? '确定',
                radius: 4,
  ...
  double getVH() {
    ...
    if (h > 600) {
      if (viewHeight > 300) viewHeight = h / 2 - b - 40;
    } else {
